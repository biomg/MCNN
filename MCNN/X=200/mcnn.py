...
def run_ideepe(parser):
    ...
    if train:
        ...
        predict3 = predict_network(model_type, np.array(X_test), channel = 2, window_size = 301 + 6, model_file = model_file+ '.301', batch_size = batch_size, n_epochs = n_epochs, num_filters = num_filters)       
            
        predict = (predict1 + predict2 + predict3)/3.0
        # print predict
	    # pdb.set_trace()
